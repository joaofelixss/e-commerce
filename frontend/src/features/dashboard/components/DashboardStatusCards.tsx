...
import {
  ...
  getOrderCounts,
  getLowStockCount,
} from "@/api/dashboard"; // Importe as funções da API
...
const DashboardStatusCards = () => {
  ...
  const [loadingData, setLoadingData] = useState(true); // Novo estado para controlar o carregamento dos dados

  useEffect(() => {
    const token = localStorage.getItem("accessToken");

    if (token) {
      setLoadingData(true); // Inicia o carregamento dos dados
      Promise.all([getTotalRevenue(), getOrderCounts(), getLowStockCount()])
        ...
        .catch((error) => {
          ...
          // Lógica para lidar com erros de busca de dados, como exibir uma mensagem ao usuário
        })
        .finally(() => {
      ...
      // Lógica para lidar com a ausência de token aqui, se necessário (embora o DashboardPage já esteja verificando)
    }
  }, []);
